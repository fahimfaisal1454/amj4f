...
import { compressImage } from "../utils/compressImage.js"; // 👈 your util
...
function CategoriesTab() {
  ...
  const del = async (id) => {
    ...
    const res = await fetch(
      ABS(`${ENDPOINTS.eventCategoriesManage}${id}/`),
    ...
    <div className="bg-white border rounded p-4">
      <form onSubmit={submit} className="grid gap-3 md:grid-cols-2">
        ...
        <div className="md:col-span-2">
          <button
            disabled={saving}
            className="rounded bg-pactPurple text-white px-4 py-2"
          >
        ...
        {items.map((c) => (
          <div key={c.id} className="rounded border p-3">
            ...
            <div className="text-xs mt-1">
              Order: {c.order} · {c.is_active ? "Active" : "Inactive"}
            </div>
...
function EventsTab() {
  ...
  const items = useMemo(
    () =>
      Array.isArray(events.data) ? events.data : events.data?.results ?? [],
    [events.data]
  );
  ...
  const submit = async (e) => {
    ...
    try {
      const url = editing
        ? ABS(`${ENDPOINTS.eventsManage}${editing.id}/`)
        : ABS(ENDPOINTS.eventsManage);
      const method = editing ? "PUT" : "POST";
      const res = await fetch(url, {
        method,
        headers: authHeaders(),
        ...
        {items.map((e) => (
          <div key={e.id} className="rounded border p-3">
            ...
            <div className="text-xs text-gray-500">
              {(cats.data || []).find((c) => c.id === e.category)?.name || ""}
            </div>
            <div className="text-xs mt-1">
              Order: {e.order} · {e.is_active ? "Active" : "Inactive"}
            </div>
            <div className="mt-2 flex gap-2">
              <button
                className="px-3 py-1 rounded bg-indigo-600 text-white"
...
function PhotosTab() {
  ...
  const items = useMemo(
    () => (Array.isArray(photos.data) ? photos.data : photos.data?.results ?? []),
    [photos.data]
  );
  ...
  // ---- CREATE (multi-upload) with compression ----
  const submit = async (e) => {
    e.preventDefault();
    const files = Array.from(fileRef.current?.files || []);
    if (!files.length) {
      alert("Pick one or more images");
      return;
    }
    if (!form.event) {
    ...
    setSaving(true);
    try {
      // compress + upload sequentially (keeps memory low, easy to debug)
      for (let idx = 0; idx < files.length; idx++) {
        ...
        body.append("image", compressed); // 👈 send compressed file
  ...
  // ---- EDIT (optional image replace) with compression ----
  const saveEdit = async (id) => {
    ...
    try {
      ...
      const picked = editFileRef.current?.files?.[0];
      if (picked) {
        const compressed = await compressImage(picked); // 👈 compress
        console.log(
          "[Replace]",
          "original:",
          (picked.size / 1024 / 1024).toFixed(2),
          "MB",
          "→ compressed:",
          (compressed.size / 1024).toFixed(0),
          "KB",
  ...
  const del = async (id) => {
    ...
    const res = await fetch(
      ABS(`${ENDPOINTS.eventPhotosManage}${id}/`),
      { method: "DELETE", headers: authHeaders(), credentials: "include" }
    );
    if (!res.ok) alert((await parseError(res)).message);
    else photos.refetch();
    ...
    <div className="bg-white border rounded p-4 space-y-4">
      ...
      <form onSubmit={submit} className="grid gap-3 md:grid-cols-2">
        <Select
          ...
          options={(events.data || []).map((ev) => ({
            value: ev.id,
            label: `${ev.title} (${ev.year || ""})`,
          }))}
          required
        />
        <Input
          label="Caption (applies to all, optional)"
          value={form.caption}
              ...
              {editingId === p.id ? (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(v) =>
                      setEditForm({ ...editForm, order: Number(v) })
                    }
                  />
                  <Checkbox
                    label="Active"
                ...
                <>
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    Order: {p.order} · {p.is_active ? "Active" : "Inactive"}
                  </div>
                  <div className="mt-2 flex gap-2">
                    <button
                      className="px-3 py-1 rounded bg-indigo-600 text-white"
