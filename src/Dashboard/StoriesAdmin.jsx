...
export default function StoriesAdmin() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const url = ABS(`${ENDPOINTS.storiesManage}${id}/`);
  ...
  return (
    <DashboardLayout>
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          onClick={openCreate}
          className="rounded bg-pactPurple px-4 py-2 text-white font-semibold hover:opacity-90"
        >
          + New Story
        </button>
      </div>
      ...
      {!loading && !error && (
        items.length === 0 ? (
          <div className="rounded border bg-white p-6 text-gray-600">
            ...
            <span className="font-semibold">“New Story”</span> to create one.
          </div>
        ) : (
...
function StoryModal({ initial, onClose, onSubmit, saving, error }) {
  ...
  const [tag, setTag] = useState(initial?.tag || "STORY");
